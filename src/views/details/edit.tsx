        ...
        <MetaWrapper title={t('meta.title')} description={t('meta.description')} keywords={t('meta.keywords')}>
            <section className='container relative mx-auto p-4 flex flex-col space-y-8'>
                <RoleGuard roles={[BusinessRoles.Super, BusinessUploadRoles.Cover]}>
                    <BusinessProfileCoverUploadSection nickName={current.business.nickName}
                                                       onOk={() => window.location.reload()}/>
                </RoleGuard>
                <RoleGuard roles={[BusinessRoles.Super, BusinessUploadRoles.Avatar]}>
                    <BusinessProfileImageUploadSection nickName={current.business.nickName}
                                                       onOk={() => window.location.reload()}/>
                </RoleGuard>
                <BusinessDetailInformationSection business={data}/>
                <BusinessEditDangerZone onOk={() => window.location.reload()} nickName={current.business.nickName}
...
export function Component() {
    return <RoleGuardView
        roles={[BusinessRoles.Super, BusinessRoles.View]}>
        <BusinessEditView/>
    </RoleGuardView>
}
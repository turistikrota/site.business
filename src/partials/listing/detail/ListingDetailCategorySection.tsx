...
const ListingDetailCategorySection: FC<Props> = ({ categoryUUIDs, features }) => {
    ...
    const { data: categories, loading : categoryLoading } = useQuery<CategoryListItem[]>(() => fetchCategoryListByUUIDs(categoryUUIDs))
    ...
    return <>
        <section>
            ...
            {fields && <>
                    {fields.inputGroups.map((group, groupIdx) => <section key={groupIdx}>
                            <FormSection.Head.Title className="text-lg font-semibold">{getI18nTranslation(group.translations, i18n.language).name}</FormSection.Head.Title>
                            <FormSection.Head.Subtitle>{getI18nTranslation(group.translations, i18n.language).description}</FormSection.Head.Subtitle>
                            <section className={'grid grid-cols-12 gap-4 mt-4'}>
                                {filterByGroup(group.uuid).map((feature, featureIdx) => <Card key={featureIdx} className={"col-span-12 md:col-span-6"}>
                                    <KeyValue>

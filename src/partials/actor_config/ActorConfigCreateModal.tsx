...
const ActorConfigCreateModal: FC<Props> = ({ open, onClose, onOk }) => {
  ...
  const form = useFormik<ActorConfigForm>({
    ...
    onSubmit: (values) => {
      setIsLoading(true)
      httpClient
        .post(apiUrl(Services.Notify, `/business/config`), {
          ...values,
        ...
        .then((res) => {
          if (res.status === 200) {
            toast.success(t('actorConfig.create.success'))
      ...
      <div>
        <Spin loading={isLoading}>
          <form onSubmit={onSubmit} className='h-full w-full'>
            <Modal.Head>
              <FormSection.Head.Title>{t('actorConfig.create.title')}</FormSection.Head.Title>
              <FormSection.Head.Subtitle>{t('actorConfig.create.subtitle')}</FormSection.Head.Subtitle>
            </Modal.Head>
            <Modal.Body>
            ...
            <Modal.Footer>
              <Button htmlType='submit'>{t('actorConfig.create.submit')}</Button>
            </Modal.Footer>
          </form>
        </Spin>
      </div>
...
export default ActorConfigCreateModal

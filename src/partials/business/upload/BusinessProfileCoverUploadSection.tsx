        ...
        <Spin loading={isLoading}>
            <div className="flex items-center justify-center">
                <img
                    ...
                    width={"100%"}
                    height={250}
                    className='object-cover w-full h-64 rounded-md bg-second border hover:shadow-md transition-shadow'
                />
            </div>
        </Spin>
        <ImageUploaderWithCropper src={src} onSrcChange={setSrc} onChange={handleUploadCover}
                                  buttonPositionClassName="right-2 bottom-2" buttonText={t('uploads.cover')}
                                  error={inputError} onError={setInputError} aspectRatio={16 / 9} circle={false}/>
    </section>
}

import { PostImage, PostLocation, PostTranslation } from '@/api/post/post.api'
import { getStaticRoute } from '@/static/page'
import { OwnerRoles, PostLogRoles, PostRoles } from '@/static/role'
...
const PostListCard: React.FC<Props> = ({
  ...
  const translation = useMemo<PostTranslation>(() => {
            ...
            <div className='grid grid-cols-12 items-center justify-between gap-2'>
              ...
              {!isValid && (
                <RoleGuard
                  ...
                  roles={[OwnerRoles.Super, PostRoles.Super, PostLogRoles.Super, PostLogRoles.List]}
                >
                  <Link to={getStaticRoute(i18n.language).owner.details.post.logs + uuid} className='col-span-6 w-full'>
              ...
              <Link
                to={getStaticRoute(i18n.language).owner.details.post.detail + uuid}
                target='_blank'

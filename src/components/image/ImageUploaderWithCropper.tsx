...
const ImageUploaderWithCropper: FC<Props> = ({
    ...
    return <>
        ...
        <span
            role='button'
            className={`absolute text-gray-800 dark:text-gray-200 flex items-center duration-200 justify-center gap-1 transition-all hover:brightness-125 bg-second border rounded-md px-3 py-1 ${buttonPositionClassName}`}
            onClick={() => {
                if (loading) return
                focus()
            }}
